...
Isaac Lab에서 학습된 Unitree H1 이족보행 정책(flat terrain)을 Isaac Sim GUI 환경에서 실시간 배포하는 데모. `H1FlatTerrainPolicy` 클래스가 69차원 observation으로부터 19차원 관절 위치 action을 추론한다. Spot(사족보행, 48D obs, 12D action)과 동일한 `PolicyController` 프레임워크를 사용하지만, DOF 수와 action scale, 물리 설정에서 차이가 있다.

이족보행은 사족보행보다 본질적으로 불안정(statically unstable)하므로, 명령 범위가 보수적(±0.75 m/s)이고 전진/회전만 지원한다(횡방향 이동 없음).
...
**Spot(48D) vs H1(69D)**: 구조는 동일하지만 DOF 차이(12 vs 19)로 인해 관절 관련 차원이 증가한다. H1의 19 DOF는 양 다리(각 6 DOF = 12) + 양 팔(각 3-4 DOF) + 허리(1 DOF) 등으로 구성된다. 추가된 상체 관절은 보행 중 균형 유지와 팔 스윙 제어에 사용된다.
...
**Action Scale = 0.5**: Spot(0.2)보다 2.5배 큰 스케일을 사용한다. H1은 이족보행으로 더 큰 관절 각도 변화가 필요하며, 특히 무릎과 발목 관절에서 넓은 운동 범위가 보행에 필수적이다.
...
H1은 `set_articulation_props=False`를 명시적으로 전달한다. 이는 PolicyController의 기본 동작(env YAML에서 solver iteration, self-collision 등을 설정)을 비활성화하고, **USD 파일에 정의된 기본 articulation 속성을 그대로 사용**한다는 의미다. Spot과 ANYmal은 이 오버라이드를 하지 않으므로 env YAML 설정이 적용된다.

이 차이는 H1 USD 에셋이 이미 적절한 solver 설정을 포함하고 있어 추가 조정이 불필요하거나, env YAML의 설정이 H1에 맞지 않기 때문일 수 있다.
...
**Spot과의 차이점**:
1. **후진/횡방향 이동 없음**: 이족보행 정책이 전진과 회전만 지원한다. 후방/횡방향 보행은 추가 학습이 필요하다.
2. **속도 범위 감소**: ±0.75 m/s (Spot은 ±2.0 m/s). 이족보행은 안정성 마진이 작아 보수적인 속도 제한이 필요하다.
3. **키 매핑 축소**: 6개 방향 → 3개 방향. 지원하는 명령 공간이 축소되었다.
...
**physics_dt = 1/200**: H1의 물리 주파수(200Hz)가 Spot(500Hz)보다 낮은 이유는 두 가지로 추정된다:
1. H1의 관절 수(19)가 Spot(12)보다 많아 물리 계산 비용이 높다
2. 이족보행의 지면 접촉 패턴이 사족보행보다 단순(2점 vs 4점)하여 상대적으로 낮은 주파수로도 충분할 수 있다

**rendering_dt = 1/25**: Spot(1/50)보다 낮은 렌더링 주파수를 사용한다. H1의 높은 DOF로 인한 렌더링 부하를 고려한 것이다.
...
**초기 높이 = 1.05m**: Spot(0.8m)보다 높다. H1의 standing height가 약 1.0m이므로, 0.05m의 여유를 두고 스폰한다. 초기 높이가 너무 낮으면 지면에 침투(penetration)하여 불안정한 시작 상태가 되고, 너무 높으면 착지 충격으로 정책이 실패할 수 있다.
