...
4대의 Franka Emika Panda가 동시에 큐브 스태킹(쌓기) 작업을 수행하는 데모. 각 로봇이 독립적인 `Stacking` Task와 `StackingController`를 가지며, y축 방향으로 2m 간격으로 배치된다. Isaac Sim의 **다중 Task 병렬 실행** 패턴을 보여주는 핵심 예제다.

단일 물리 루프(`_on_physics_step`)에서 4개의 독립적인 Task-Controller 쌍을 동기적으로 실행하는 구조로, 다중 로봇 시뮬레이션의 가장 기본적인 스케일링 패턴을 학습할 수 있다.
...
`get_observations()`를 **한 번만** 호출하여 씬 전체의 상태를 가져오고, 각 Controller가 자신의 로봇/큐브에 해당하는 key만 참조한다. 이 패턴은 observation 수집 비용을 O(N)에서 O(1)로 줄인다.
...
`offset`으로 각 Task를 y축 방향으로 분리한다: `[-3, -1, 1, 3]`m. 각 Stacking Task가 독립적인 Franka + 큐브 세트를 생성하므로, 4개의 완전히 독립적인 workspace가 만들어진다.

Task 간 물리적 간섭이 없도록 2m 간격을 두는데, Franka의 reach 범위(약 0.855m)를 고려하면 workspace 겹침이 발생하지 않는다.
...
**순차 action 계산 → 일괄 물리 시뮬레이션**: 4대 로봇의 action을 순서대로 계산하고 적용한다. `apply_action()`은 PhysX에 명령을 큐잉할 뿐, 실제 물리 스텝은 이 콜백 함수가 끝난 후 한 번에 실행된다. 따라서 4대 로봇의 action이 동일한 물리 타임스텝에서 동시에 적용된다.

### StackingController 상태 머신

각 Controller는 내부적으로 다음 상태를 순환한다:
...
`picking_order_cube_names`로 큐브 스태킹 순서를 지정한다. 각 Controller가 독립적인 상태를 유지하므로, 4대 로봇이 서로 다른 진행 상태에 있을 수 있다.
...
- `_num_of_tasks`를 변경하면 로봇 수를 늘릴 수 있다. GPU 메모리가 허용하는 한 수십 대까지 스케일링 가능.
- offset 간격을 줄이면 로봇 간 물리적 간섭이 발생한다 — 충돌 회피가 필요한 시나리오를 테스트할 때 활용.
- Isaac Lab의 병렬 환경(4096+ 동시 실행)은 이 패턴의 GPU 가속 버전이다.
