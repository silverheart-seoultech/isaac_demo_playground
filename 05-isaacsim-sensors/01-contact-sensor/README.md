...
PhysX 기반 접촉 센서(Contact Sensor)의 생성, 구성, 데이터 수집을 다루는 예제. Ant 로봇의 4개 발(foot)에 접촉 센서를 부착하여, 각 발이 지면에 가하는 힘을 실시간으로 측정한다.

Contact Sensor는 지정된 구형(spherical) 영역 내에서 body surface에 가해지는 모든 힘의 합(net force)을 반환한다. Isaac Sim에서 가장 기본적인 센서 타입으로, 보행 로봇의 발 접촉 감지, 그리퍼의 파지력 측정 등에 활용된다.
...
Kit Command는 USD stage에 직접 prim을 생성하는 저수준 방식이고, Python API(`ContactSensor` 클래스)는 이를 래핑한 고수준 인터페이스다. Standalone 스크립트에서는 Python API가 더 적합하다.
...
`radius`는 센서의 감지 범위를 결정한다. 이 구형 영역이 body surface와 교차하는 부분의 접촉력만 합산한다. radius가 너무 작으면 접촉을 놓치고, 너무 크면 인접 body의 접촉까지 잡힌다.
...
`acquire_contact_sensor_interface()`는 PhysX의 C++ Contact Sensor 인터페이스에 대한 Python 바인딩을 획득한다. `reading.value`는 kg⋅m⋅s⁻² 단위(= N)이며, stage의 meters_per_unit을 곱해서 일관된 단위로 변환한다.
...
`add_raw_contact_data_to_frame()`을 호출하면 매 프레임에 raw contact 데이터(접촉점, 법선, 충격량 등)가 포함된다. `get_current_frame()`은 dictionary로 센서 데이터를 반환한다.
...
Extension 버전은 `Arm_XX/Lower_Arm` 하위에 센서를 부착하고, Standalone 버전은 `{foot_name}` 하위에 부착한다. 같은 물리적 위치를 다른 USD 경로로 참조하는 것인데, 이는 Extension과 Standalone에서 사용하는 USD 파일이 약간 다르기 때문이다 (`ant_colored.usd` vs `ant.usd`).
...
Contact Sensor는 Isaac Lab의 보행 환경에서 핵심적으로 사용된다:
...
Isaac Lab에서는 `ContactSensorCfg`로 선언적 구성이 가능하다:
