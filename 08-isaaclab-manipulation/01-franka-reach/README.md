...
Franka Emika Panda 로봇으로 end-effector의 목표 pose를 추종하는 reaching 환경. Isaac Lab manipulation의 가장 기본적인 태스크로, 물체 접촉 없이 end-effector 위치/자세만 제어한다. 4가지 action space variant(joint_pos, ik_abs, ik_rel, osc)를 제공하여 **같은 태스크를 다른 action representation으로 학습하는 비교 실험**이 가능하다.

Reach 환경은 locomotion의 velocity tracking과 유사하게, 공통 베이스(`ReachEnvCfg`)를 로봇별/action type별로 상속하는 구조다.
...
Locomotion 대비 observation이 매우 단순하다(32~34D). Reaching은 로봇 자체의 상태와 목표만 알면 되므로 지형 정보나 접촉 센서가 불필요하다.
...
**Coarse + Fine 이중 추종 전략**: `position_tracking(-0.2)`은 L2 거리로 넓은 범위에서 끌어당기고, `position_tracking_fine(+0.1)`는 tanh로 목표 0.1m 이내에서 sharply rewarding한다. 이 조합은 빠른 접근 + 정밀한 도달을 모두 학습하게 한다.
...
`action_rate`의 가중치가 -0.0001에서 4500 스텝에 걸쳐 -0.005로 50배 증가한다. 학습 초기에는 penalty 없이 자유롭게 탐색하다가, 수렴 단계에서 부드러운 움직임을 강제한다. 이 curriculum 패턴은 "먼저 해결법을 찾고, 그 다음 세련되게 만든다"는 전략이다.
...
`pitch=(π, π)`는 end-effector가 항상 아래를 향하도록 고정한다 (Franka의 z축이 panda_hand에서 아래 방향). `yaw`만 자유도를 주어 도달 가능한 자세 공간을 제한한다.
...
Locomotion(200Hz, decimation 4)보다 낮은 물리 주파수다. Reaching은 빠른 동적 제어가 필요 없으므로 60Hz로 충분하다.
...
네트워크가 [64,64]로 locomotion([512,256,128])보다 훨씬 작다. Reaching은 7D 관절 → 6D pose의 단순한 매핑이므로 작은 네트워크로 충분하다.

## Comparison: Action Space Variants

같은 reaching 태스크를 4가지 action representation으로 학습할 수 있다:
...
이 비교는 RL에서 action representation 선택이 학습 효율과 태스크 성능에 미치는 영향을 이해하는 데 핵심적이다.
