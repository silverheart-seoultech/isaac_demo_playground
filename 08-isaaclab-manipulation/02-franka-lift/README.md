...
Franka Panda 로봇으로 큐브를 집어 목표 위치로 들어올리는 환경. Reach 환경의 확장으로, **gripper 제어 + 물체 상호작용**이 추가된다. Observation에 물체 위치와 목표 위치가 포함되고, reward는 접근 → 잡기 → 들기 → 목표 추종의 단계적 구조를 가진다.

3가지 action space variant(joint_pos, ik_abs, ik_rel)를 제공한다. Reach와 달리 gripper action이 Binary(열기/닫기)로 추가된다.
...
Reach 대비 `object_position`(물체의 현재 위치, 로봇 base frame 기준)과 `target_object_position`(물체 목표 위치)이 추가된다. 정책은 물체가 어디에 있고 어디로 가야 하는지를 직접 관찰한다.

`object_position_in_robot_root_frame`는 물체 위치를 world frame이 아닌 로봇 root frame으로 변환하여 반환한다. 이는 병렬 환경에서 각 환경의 절대 좌표가 다르므로, 로봇 기준 상대 좌표를 사용하는 것이 학습에 유리하기 때문이다.
...
**단계적 Reward 설계**: 이 reward 구조는 정책이 자연스럽게 아래 순서로 학습하게 만든다:
...
`min_height=0.04` 게이트는 핵심 설계이다. 물체를 들어올리기 전에는 goal tracking reward가 0이므로, 정책은 먼저 접근과 들기에 집중한다. 들어올린 후에야 goal tracking이 활성화되어 이동을 학습한다.
...
Gripper를 연속 제어 대신 Binary(열기/닫기)로 단순화한다. 정책이 gripper 제어에 대한 학습 부담을 줄이고 물체 접근/이동에 집중할 수 있다. 실제 pick-and-place에서도 gripper는 대부분 완전 열기/닫기로 사용된다.
...
`panda_link0`(base)에서 `panda_hand`까지의 변환을 추적하며, 추가로 z방향 0.1034m offset을 적용하여 실제 finger tip 위치를 반환한다. 이 변환은 물체와 ee 간 거리 계산에 사용된다.
...
물체 위치를 매 에피소드 x±0.1m, y±0.25m 범위에서 랜덤화한다. 높이(z)는 테이블 위에 고정. 정책이 다양한 물체 위치에서 잡기를 학습하게 한다.
...
접촉이 포함되므로 Reach(60Hz)보다 높은 100Hz 물리를 사용한다. 접촉 해석의 안정성을 위해 `friction_correlation_distance=0.00625`, `solver_position_iteration_count=16`을 설정하여 물체 파지의 물리적 안정성을 확보한다.
...
학습 시 `action_rate`와 `joint_vel`의 penalty weight가 10000 스텝에 걸쳐 -1e-4에서 -1e-1로 **1000배** 증가한다 (Reach는 50배). Lift는 물체 조작의 정밀도가 중요하므로 후반부에 매우 강한 smoothness 제약을 건다.

## Comparison: Reach → Lift 확장 패턴

Lift는 Reach에서 아래 요소를 추가하여 구현된다:
...
이 패턴은 Isaac Lab에서 새로운 manipulation 태스크를 설계할 때 참고할 수 있는 확장 가이드라인이다.
